...
def cycle(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(lr, "build_event_filter", lambda cfg: SimpleNamespace(
    ...
    class FakeBroker:
        ...
        def quantize(self, inst, qty):
            ...
            return round(math.floor(abs(qty) / step) * step, decimals)
    ...
    return SimpleNamespace(lr=lr, broker=broker, journal=journal, signals=signals,
                           memory_dir=memory_dir, requested=requested)
